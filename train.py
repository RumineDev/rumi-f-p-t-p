...
def main(args):
    ...
    # Disable wandb by default in Kaggle environment
    ...
    if args['distributed']:
        # For distributed training, use DistributedSampler with class balancing
    ...
    # Use differential learning rates if we have both backbone and head params
    use_differential_lr = len(backbone_params) > 0 and len(head_params) > 0
    ...
    if args['optimizer'] == 'AdamW':
        ...
        if use_differential_lr:
            ...
            print(f"Using SGD for {args['model']} with uniform LR: {args['lr']}")
    else:
        # default behavior
        if 'dinov3' in args['model']:
            if use_differential_lr:
                ...
                optimizer = torch.optim.AdamW(param_groups)
                print(f"Using default AdamW for {args['model']} with differential LR")
            else:
                params = [p for p in model.parameters() if p.requires_grad]
                optimizer = torch.optim.AdamW(params, lr=args['lr'], weight_decay=1e-4)
            ...
            if use_differential_lr:
                ...
                params = [p for p in model.parameters() if p.requires_grad]
                optimizer = torch.optim.SGD(params, lr=args['lr'], momentum=0.9, nesterov=True)
                print(f"Using default SGD for {args['model']} with uniform LR: {args['lr']}")
